...
read -r lastLinuxEvalNo lastLinuxEvalTime <<<"$(
  set -e
  runRust crawl_jobset nixos "${nixosJobset}"
)"

echo "Asking Hydra about nixpkgs..."
read -r lastDarwinEvalNo lastDarwinEvalTime <<<"$(
  set -e
  runRust crawl_jobset nixpkgs "${nixpkgsJobset}"
)"
